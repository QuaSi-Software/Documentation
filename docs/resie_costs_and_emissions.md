...
The annuity is a calculated constant yearly value that is economically equivalent to all costs and revenues occurring over the observation period under the assumed interest rate. It annualizes one-time, irregular, and recurring cashflows, such as investments, operating costs, replacements, revenues, and residual values, into one representative annual amount. It is not necessarily a cashflow that occurs in reality, but an annual equivalent value whose discounted sum corresponds to the present value of the actual cashflow sequence. The annuity can therefore be interpreted as the constant yearly cost or benefit of owning and operating the energy system over the observation period. It can be used to compare different system concepts on a common annual basis, even if their investments, operating costs, revenues, and component lifetimes differ.

Beside the annuity, also cashflows and present values are calculated. A present value is the value at the beginning of the observation period that is economically equivalent to a future cashflow under the assumed interest rate. So each future cost or revenue is discounted according to its timing. Cashflows occurring later in the observation period therefore contribute less to the total present value than equal nominal cashflows occurring earlier. The sum of all present values represents the total economic value of the cashflow series at the start of the observation period and forms the basis for the annuity calculation.
