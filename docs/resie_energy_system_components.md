...
![Energy flow within the thermal booster](fig/260210_thermal_booster.svg)
...
1. **Low-temperature heat** from an external source (e.g. a storage or a district heating system).
2. **Additional energy**, which is converted into heat inside the booster (e.g. electrical energy).
...
The demand returns (input into the thermal booster) with a temperature \(T_{\text{ht,in}}\) [°C] and should be delivered at a higher outlet temperature \(T_{\text{ht,out}}\) [°C]. The working fluid has an (assumed constant) specific heat capacity \(c_{p,\text{out}}\) [J/(kg·K)], and the mass flowing through the booster during that step is \(m\) [kg].

The useful heat supplied to the demand in that time step is then
\[
Q_{\text{heat,ht,out}} = m \, c_{p,\text{out}} \,(T_{\text{ht,out}} - T_{\text{ht,in}}),
\]
where \(Q_{\text{heat,ht,out}}\) [Wh] is the useful heat delivered to the demand. This heat \(Q_{\text{heat,ht,out}}\) is composed of two contributions: low-temperature heat taken from an external source and heat generated by boosting.

Denoting \(Q_{\text{heat,lt,in}}\) [Wh] as the heat taken from the low-temperature source and \(Q_{\text{boost}}\) [Wh] as the heat added by the thermal booster, the energy balance of the component is
\[
Q_{\text{heat,ht,out}} = Q_{\text{heat,lt,in}} + Q_{\text{boost}}.
\]

In thermodynamic terms, the thermal booster first preheats the fluid from the demand return temperature \(T_{\text{ht,in}}\) up to some intermediate temperature \(T_{\text{int}}\) [°C] using low-temperature heat. Then it uses additional energy to lift the temperature from \(T_{\text{int}}\) up to the desired outlet temperature \(T_{\text{ht,out}}\).

The low-temperature contribution is
\[
Q_{\text{heat,lt,in}} = m \, c_{p,\text{out}} \, (T_{\text{int}} - T_{\text{ht,in}}),
\]
and the additional boosting contribution is
\[
Q_{\text{boost}} = m \, c_{p,\text{out}} \, (T_{\text{ht,out}} - T_{\text{int}}).
\]

Here, \(T_{\text{int}}\) [°C] is the intermediate temperature after preheating but before boosting.

Using these definitions, the total useful heat
\[
Q_{\text{heat,ht,out}} = m \, c_{p,\text{out}} \, (T_{\text{ht,out}} - T_{\text{ht,in}})
\]
is automatically satisfied by the sum of preheating and boosting. In the model, \(T_{\text{int}}\) is chosen such that it respects the physical limits of the low-temperature source (in particular a minimum terminal temperature difference to the source), and such that the available low-temperature energy and available boost energy are not exceeded. From the thermodynamic point of view, this just determines how much of \(Q_{\text{heat,ht,out}}\) is taken from the low-temperature side and how much must be provided by additional energy.

Inside the thermal booster, the additional heater is treated as an ideal thermal converter before losses: every unit of net boost energy used by the heater appears as the same amount of heat \(Q_{\text{boost}}\) added to the fluid. We therefore identify
\[
Q_{\text{boost}} = Q_{\text{boost,net}},
\]
where \(Q_{\text{boost,net}}\) [Wh] is the additional energy effectively converted into heat inside the fluid and \(Q_{\text{boost}}\) [Wh] is the boosting heat that raises the fluid temperature from \(T_{\text{int}}\) to \(T_{\text{ht,out}}\).

The actual additional power rating of the booster, \(P_{\text{boost,max}}\) [W], limits the maximum \(Q_{\text{boost,net}}\) in one time step via
\[
Q_{\text{boost,net}} \le P_{\text{boost,max}} \, \Delta t,
\]
with \(\Delta t\) [s] the simulation time step.
...
- \(\eta_{\text{boost}}\) for losses on the additional boost energy side (for example electrical losses, internal conversion losses),
- \(\eta_{\text{heat}}\) for thermal losses with respect to the low-temperature thermal input.

If we denote by \(Q_{\text{boost,gross}}\) [Wh] the gross boost energy taken from the surrounding system and by \(Q_{\text{heat,in,gross}}\) [Wh] the gross low-temperature heat taken from external sources, then the net useful energies actually available inside the booster are
\[
Q_{\text{boost,net}} = \eta_{\text{boost}} \, Q_{\text{boost,gross}}, \qquad
Q_{\text{heat,in,net}} = \eta_{\text{heat}} \, Q_{\text{heat,in,gross}}.
\]

With the identification
\[
Q_{\text{boost,net}} = Q_{\text{boost}}, \qquad
Q_{\text{heat,in,net}} = Q_{\text{heat,lt,in}},
\]
the total useful output heat becomes
\[
Q_{\text{heat,ht,out}} = Q_{\text{heat,lt,in}} + Q_{\text{boost}}
               = \eta_{\text{heat}} \, Q_{\text{heat,in,gross}}
               + \eta_{\text{boost}} \, Q_{\text{boost,gross}}.
\]

The difference between gross and net energies represents the losses of the component. The additional boost energy losses are
\[
Q_{\text{loss,boost}} = Q_{\text{boost,gross}} - Q_{\text{boost,net}}
                      = (1 - \eta_{\text{boost}})\, Q_{\text{boost,gross}},
\]
and the thermal losses associated with using low-temperature heat are
\[
Q_{\text{loss,heat}} = Q_{\text{heat,in,gross}} - Q_{\text{heat,in,net}}
                     = (1 - \eta_{\text{heat}})\, Q_{\text{heat,in,gross}}.
\]

The total losses of the thermal booster in one time step are then given by
\[
Q_{\text{loss,total}} = Q_{\text{loss,boost}} + Q_{\text{loss,heat}}.
\]
...
- `allow_boost_solely` enables boost-only operation. If this flag is `true`, the thermal booster may deliver heat even when no low-temperature heat is available (\(Q_{\text{heat,lt,in}} = 0\)), i.e. \(Q_{\text{heat,ht,out}} = Q_{\text{boost}}\). If it is `false`, the component requires a non-zero low-temperature contribution to operate; without \(Q_{\text{heat,lt,in}}\), no heat is supplied, even if boost energy would be available.

- `allow_boost_additional` controls how strictly the low-temperature source has to do the heating.
If this flag is `false`, the low-temperature side must always heat the demand as far as possible (up to its maximum feasible intermediate temperature). The booster is then only allowed to do the remaining temperature lift to \(T_{\text{ht,out}}\). In this case, the delivered heat \(Q_{\text{heat,ht,out}}\) is essentially limited by how much low-temperature heat is available; boost energy cannot compensate a lack of \(Q_{\text{heat,lt,in}}\).
If this flag is `true`, the low-temperature side is not forced to provide the maximum possible temperature lift. Whenever the low-temperature heat is not sufficient to reach the required mass flow to satisfy the desired energy demand, the model can use additional boost energy to cover the missing part. This allows the booster to deliver the required  \(Q_{\text{heat,ht,out}}\) even without enough low-temperature energy, as long as boost power and demand limits are respected.
...
Symbol | Description | Unit
------ | ----------- | ----
\(c_{p,\text{out}}\) | Specific heat capacity of the output medium (assumed constant) | [J/(kg·K)]
\(m\) | Mass of fluid passed through the booster in the considered time step | [kg]
\(P_{\text{boost,max}}\) | Maximum additional (boost) power rating of the booster | [W]
\(Q_{\text{boost}}\) | Net useful boosting heat added by the booster in the considered time step | [Wh]
\(Q_{\text{boost,gross}}\) | Gross boost energy drawn from the additional energy side (e.g. electrical grid) | [Wh]
\(Q_{\text{boost,net}}\) | Net boost energy effectively converted into heat inside the fluid | [Wh]
\(Q_{\text{heat,ht,out}}\) | Useful heat delivered to the demand in the considered time step | [Wh]
\(Q_{\text{heat,in,gross}}\) | Gross low-temperature heat taken from external thermal sources | [Wh]
\(Q_{\text{heat,in,net}}\) | Net low-temperature heat effectively used for preheating | [Wh]
\(Q_{\text{heat,lt,in}}\) | Net useful heat taken from the low-temperature source in the considered time step | [Wh]
\(Q_{\text{loss,boost}}\) | Losses associated with the additional boost energy input | [Wh]
\(Q_{\text{loss,heat}}\) | Losses associated with low-temperature heat input | [Wh]
\(Q_{\text{loss,total}}\) | Total losses of the thermal booster (boost plus thermal losses) | [Wh]
\(T_{\text{ht,in}}\) | Return (input) temperature of the demand at the inlet of the thermal booster | [°C]
\(T_{\text{ht,out}}\) | Outlet (supply) temperature delivered by the thermal booster to the demand | [°C]
\(T_{\text{int}}\) | Intermediate temperature after preheating with low-temperature heat and before boosting | [°C]
\(T_{\text{lt,in}}\) | Input temperature of the low temperature supply | [°C]
\(\Delta t\) | Duration of the simulation time step | [s]
\(\eta_{\text{boost}}\) | Efficiency factor for the additional boost energy path | [-]
\(\eta_{\text{heat}}\) | Efficiency factor for the low-temperature heat path | [-]
...
$$\dot{Q}_3 = A_{x,z} \; \lambda_{soil} \; \frac{(T_{i,j-1} - T_{i,j})}{\Delta y_{j-1}} = \frac{(\Delta x_{i-1} + \Delta x_i)}{2} \; \Delta z \; \lambda_{soil} \; \frac{(T_{i,j-1} - T_{i,j})}{\Delta y_{j-1}}$$
...
\(a_{P0}, a_{P}, a_{W}, a_{E}, a_{N}, a_{S}\) | linear system coefficients (storage & face conductances) | [W / K]]
...
\(G_{W}, G_{E}, G_{N}, G_{S}\) | face thermal conductances | [W / K]
...
- **Ground surface towards ambient air** (\(z=0\)): Robin condition
  ...
  \[  h(r)=  \begin{cases}
    ...
    h_s, & \text{elsewhere},
  \end{cases}  \]
  where \(R_s\) is the (area-equivalent) storage radius at the surface, \(w_{\text{ov}}\) is the overlap width, \(U_{\text{ov}}\) the effective surface U-value of the top insulation overlap ring and  \(h_s\) the effective surface heat transfer coefficient outside of the top insulation.
...
- **Storage wall (buried part)**: Robin condition on the exterior soil side,
  ...
  \(U_w\) is the effective wall heat-transfer coefficient (U-value) between the storage medium  and the adjacent soil, i.e. it lump-sums the thermal resistance of the wall/insulation and any near-wall contact
...
A cell-centered finite-volume scheme with axisymmetric metrics is used (see also the description of the [geothermal heat collector](#geothermal-heat-collector)):
...
3. **Above-ground side and lid:** these exchange with the ambient air, i.e. their effective ambient temperature equals the ambient air temperature. Note that no radiation is included for now.
