...
3. The transformer at the end now performs the `process` operation, which is possible as now the whole energy system is specified in regards to energy potentials.
...
- In some cases, components can be considered multiple times if they are part of multiple branches at one MB or MT! Therefore, after the order has been determined, only the first appearance of the process is considered and all other process operations of the same component are deleted from the order of operation. For potential operation, this is not done as in some cases, components have to have multiple potential operations. If one component has its potential and right afterwards its process operations, the potential operation is deleted as it is effectively the same at this point. This is repeated until there are no more changes in the order of operation. 
