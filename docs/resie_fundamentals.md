...
7. Reorder the `Process` step of flexible sources and flexible sinks and the `Process` / `Load` step of storages that are directly (meaning without an intermediate bus) connected to transformers such that their steps always come after the `Process` step of the connected transformer.
