...
* `change_bus_priorities`: Changes the input and output priorities on a bus as well as changing the energy flow matrix.
* `reorder_operations`: Rearranges the order of operations for a timestep of the simulation.

**Note:** The callbacks `change_bus_priorities` and `reorder_operations` are called outside of any `control` operation at the beginning of a timestep before any operations are performed. They are also both required by any control module implementing them to ensure correct calculation.

**Note:** The callbacks `reorder_inputs`, `reorder_outputs`, `determine_temperature_and_energy`,  `cooling_input_temperature_exceeded`, `change_bus_priorities` and `reorder_operations` are not aggregated. They are evaluated one module at a time in the order they appear in the component config within the project file. Which module takes effect and which modules are overwritten depends on the control module implementation. In general this case should be avoided.
...
* `economic_control`: Reorders priorities and changes the energy flow matrix on a bus depending on a price profile and a threshold value.
