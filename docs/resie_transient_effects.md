...
In the following the calculations of maximal consumable gas and maximal produced heat for the GB for different operating strategies are presented in more detail. For full load there are no additional calculation needed, as maximal consumable gas is equal to maximal produced heat. Before diving deeper into the computation steps, it should be noted that for the thermal efficiency curve of the gas boiler a polynomial fit is used, which resembles the grey curve with the only difference that implemented curve is monotonic on the whole PLR interval. The default values for the coeffcients were taken from the LeeSeo2019[^LeeSeo2019] paper. According to the paper the values were obtained by performing different regression procedures based on gas boiler usage data for a US company.  
For supply driven strategy the intake amount of gas is given at the current time step. To obtain the part load ratio the yellow curve is inverted, whereby the inversed function takes the provided given amount of gas and the maximal consumable gas as input. The maximal consumable gas is obtained by solving the energy balance equation, in other words the rated power is divided by the maximal themal efficiency value. Next step is to compute the thermal efficiency (grey function). Last but not least, the energy balance equation is solved to obtain the max producable heat for the obtained thermal efficiency of the gas boiler. This is simply done by multiplying the thermal efficiency with maximal consumable gas.

When a GB is operated through a demand driven strategy, the maximal produced heat and maximal consumable gas are determined as follows. First, the maximal produced heat is set to be equal to the rated power, instead of the demanded power. This shall prevent the case where the demand power is infinite which in hand will lead to a non-physical state. Second, the demand heat is checked whether it exceeds the maximum producable heat. If this is the case, then the demand heat is limited to max produce heat [^min_use_frac]. Afterwards, the part load ratio is calculated via \(PLR(E_{useful}) = \frac{E_{useful}}{E_{useful,rated}}\). Thermal efficiency is calculated by evaluating the pre-implemented function (orange curve) with the obtained PLR as input. Last but not least, the maximal consumable gas is computed by solving the energy balance equation. In this case, the maximal produced heat is divided by the computed thermal efficiency.
