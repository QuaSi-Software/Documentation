...
The battery temperature wasn't available from the measurement data and since ReSiE currently only supports a constant temperature the temperature was varied to get a good fit to the measurement and a value of 35 °C seamed to get good results, while being in a realistic range. The tests also showed a high impact of the temperature on the model results, so it should be chosen carefully.

In the first step ReSiE was run with a time step of one second to show the behaviour if all the peaks and fast changes are present in the data. Additionally ReSiE and SAM&trade; were run with 5 miniutes, 15 minutes and 1 hour time steps to show the behaviour in a more realistic usecase.
In table below you can see the results for the comparission of ReSiE and the measurement data. It shows the mean error (ME) for the cell voltage \(V_{cell}\) and the \(SOC_{adj}\) during operation as well as the difference in round trip efficiency \(\eta_{rte}\) between two points with the same SOC_{adj} at the beginning and the end of the month.
...
Another reason for discrepancies between measurements and ReSiE could be the use of a constant temperature. In reality the temperatures of the cells are still fluctating even if they are temperature controlled. This might be still improved in future versions of ReSiE.

In the figure below you can see the comparison between ReSiE and the measurement data at a 1 second time step. On the left axis is the \(SOC\) and on the right axis \(V_{cell}\). Here it can be clearly seen how much variance there is in the charging and discharging power, leading to high fluctuation in the cell voltage. ReSiE is able to follow that quite closely. It is also visible that the voltage relaxation is not modeled in ReSiE (e.g. after 03.04.2023 12:15). In ReSiE the voltage immediately becomes stable after the power goes to 0, compared to the measurement data where the voltage slowly falls off. Since this happens only during times without charging or discharging power, it doesn't have much impact on the battery efficiency.
...
The results show that SAM&trade; has a worse performance compared to ReSiE while being overall more consistent between different time steps. This can be attributed to the current dependend capacity in ReSiE which has a greater impact at smaller time steps with higher variability in the charging and discharging power. At a time step of 1 h the two models perform almost identical, even though each model uses it's own parameters for the LFP cell chemistry. 
That shows that the impact of differences in specific cells is negligible and as long as the right general chemistry is selected.
